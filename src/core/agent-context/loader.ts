...
/**
 * 从 Agent Home 根目录读取上下文文件，返回 ContextFile[]。
 *
 * 行为规则：
 ...
 *
 * @param agentHome - Agent Context 根目录路径
 * @param opts - 可选配置
 */
 ...
 * **不**读白名单之外的文件。
 *
 * 与 `loadContextFiles(agentHome, opts)` 的区别：
 *  - 直接读 `<absDir>/<filename>`
 *  - 调用方负责传完整路径（例如 Child executor 传 `<agentHome>/subagents/<id>/`）
 *
