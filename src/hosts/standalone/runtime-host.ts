import type { RuntimeHandle, RuntimeShutdownReport } from '../../runtime/index.js';
...
export function createRuntimeHost(
  ...
  let resolveCompletion!: (report: RuntimeShutdownReport) => void;
  let rejectCompletion!: (error: unknown) => void;
  const completion = new Promise<RuntimeShutdownReport>((resolve, reject) => {
  ...
  return Object.freeze({ shutdown, completion, dispose });
}