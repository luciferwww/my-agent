...
describe('FT-03 Runner input boundary', () => {
  ...
  it('keeps Runtime and Runner leaf configuration owned by their modules', async () => {
    const productionSources = await loadProductionSources(REPOSITORY_ROOT);
    const [platformTypes, platformDefaults, configLoader, runtimeConfig, runnerConfig, runner] =
      await Promise.all([
        readFile(join(REPOSITORY_ROOT, 'src', 'platform', 'config', 'types.ts'), 'utf8'),
        readFile(join(REPOSITORY_ROOT, 'src', 'platform', 'config', 'defaults.ts'), 'utf8'),
    ...
    expect(platformDefaults).not.toContain('DEFAULT_RUNTIME_CONFIG');
    expect(platformDefaults).not.toContain('DEFAULT_RUNNER_CONFIG');
    expect(platformDefaults).not.toContain('maxLlmCalls: 12');
