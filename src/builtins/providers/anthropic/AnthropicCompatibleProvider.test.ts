...
function resolve(provider: AnthropicCompatibleProvider, modelId: string, tools = false) {
...
describe('AnthropicCompatibleProvider', () => {
  it('uses exact Provider-owned static facts for a current model on the official endpoint', () => {
    const provider = new AnthropicCompatibleProvider({ apiKey: 'test-key' });
  ...
  it('rejects invalid or non-Anthropic deployment-facts input at startup', () => {
    expect(() => new AnthropicCompatibleProvider({
      apiKey: 'test-key',
      deploymentFacts: [{
        providerId: 'other',
