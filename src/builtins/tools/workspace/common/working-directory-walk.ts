...
export type WorkingDirectoryFileEntry = {
  absolutePath: string;
  relativePath: string;
};

export async function listWorkingDirectoryFiles(workingDir: string): Promise<WorkingDirectoryFileEntry[]> {
  const files: WorkingDirectoryFileEntry[] = [];
  ...
  async function walk(currentDir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      files.push({
        absolutePath,
        relativePath: normalizeForDisplay(relative(workingDir, absolutePath)),
      });
    }
  }
