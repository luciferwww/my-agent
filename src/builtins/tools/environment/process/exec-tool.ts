...
export function createExecTool(agentHome: string): Tool {
  return {
  name: 'exec',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      cwd: {
        type: 'string',
        description: 'Execution context for the command. Defaults to Agent Home and does not confine command effects.',
      },
      env: {
        type: 'object',
  ...
  execute: async (params, context) => {
    const normalized = normalizeExecRequest(params, agentHome);
