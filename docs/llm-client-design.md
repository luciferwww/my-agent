...
> 创建日期：2026-04-02  
> 参考：OpenClaw 的 `pi-ai` 库（底层 LLM 调用）+ `pi-agent-core`（Agent 循环）
...
pi-agent-core（中间层）
  └── Agent.prompt() → 内部调用 StreamFn → Agent 循环

openclaw（应用层）
  └── attempt.ts → 选择 StreamFn → 传给 Agent
```
...
| | OpenClaw | 我们 |
|---|---|---|
| 抽象层 | `StreamFn` 函数类型 | `LLMClient` 接口 |
| Provider 数量 | 10+（anthropic, openai, google, ollama...） | 1（anthropic，将来扩展） |
| 流式方式 | `StreamFn` 返回 `EventStream` | `chatStream` 返回 `AsyncIterable` |
| 非流式 | 无（全部流式，Agent.prompt 内部收集） | `chat` 便捷方法（内部调 chatStream 收集） |
| 类型依赖 | 各层独立（pi-ai 有自己的 Message 类型） | 各层独立（llm-client 自己定义类型） |
| 自定义 baseURL | ✅ provider 配置 | ✅ 构造参数 |
...
| 模型 Fallback | 主模型失败切换备用 | OpenClaw `model-fallback.ts` |
| 速率限制 | 高并发场景 | 令牌桶或信号量 |
