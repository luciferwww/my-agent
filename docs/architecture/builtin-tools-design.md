...
当前 builtin tools 的命名规则采用“分类统一”而不是“全局统一语序”：

- 文件读写修改类倾向 `verb_resource`：如 `read_file`、`write_file`、`edit_file`
- 搜索 / 抓取类倾向 `resource_action`：如 `file_search`、`memory_search`、`web_fetch`
- 同一能力家族保持共同前缀：如 `memory_search`、`memory_get`、`memory_write`

这比把所有工具都改成名词前置或动词前置更稳，因为不同类型的工具有不同的主要心智模型。
...
`grep_search` 当前也建议保留：它的重点不是“名字是否最优雅”，而是能明确表达文本 / 正则匹配这类 grep-like 语义。
