...
| | OpenClaw | 我们 |
|---|---|---|
| 入口 | `agentCommandInternal()` → `runEmbeddedAttempt()`（2800+ 行） | `AgentRunner.run()`（精简） |
| Tool loop | 在 `pi-agent-core` 库内部 | 自己实现 while 循环 |
| streamFn 包装 | 7+ 层包装链 | 不需要，直接调 llm-client |
| 事件机制 | subscribe 订阅模式 | 回调函数（`onEvent`） |
| 工具执行 | 内置 tools + MCP + LSP | 回调函数（`toolExecutor`），调用方自定义 |
| 模型 fallback | `runWithModelFallback()` | 暂不支持 |
| Compaction | 自动检测 + 重试 | 暂不支持 |
...
每条消息在产生时立即保存到 session（与 OpenClaw 一致），不是等全部结束后批量保存：
...
| 模型 fallback | 主模型失败 | OpenClaw `model-fallback.ts` |
| AbortSignal | 用户取消 | 传递给 llm-client |
| 历史裁剪 | token 超限 | OpenClaw `limitHistoryTurns()` |
| 对话压缩 | 上下文溢出 | OpenClaw `compact.ts` |
