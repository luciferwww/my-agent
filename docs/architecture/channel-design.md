...
### 9.2 `src/agent-runner/AgentRunner.ts` 与 `types.ts`
...
```typescript
// src/agent-runner/test-helpers.ts
...
| 维度 | 文件 | 改动 |
|------|------|------|
| 类型 | `src/agent-runner/hooks/types.ts` | `BeforeToolCallPayload` 加 `turnId` + `sessionKey`（§9.1） |
| 类型 | `src/agent-runner/types.ts` | `RunParams` 加 `turnId`（必填）；`AgentEvent` 每个变体单独声明 `sessionKey` + `turnId`（不引入公共基类型）（§9.2） |
| 类型 | `src/runtime/types.ts` | `RunTurnParams` 加 `turnId?`；`RuntimeAppOptions` 加 `onAgentEvent?`（§9.4） |
| 行为 | `src/agent-runner/AgentRunner.ts` | `emit()` 注入路由字段；hook payload 注入 `turnId`/`sessionKey`；暴露 `on()` 注册 API（§9.2） |
...
| 新增 | `src/agent-runner/test-helpers.ts` | `makeRunParams()` helper（§9.2） |
| 测试 | `src/agent-runner/*.test.ts` | runner.run 调用补 `turnId`；AgentEvent 断言用 `toMatchObject`（§9.2） |
