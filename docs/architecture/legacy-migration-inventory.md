...
现有行为 Specs 在其行为被新的 Accepted Spec/实现替代前继续生效，包括 [Core Abort Spec](core-abort-spec.md)、[Multi-client User Message Spec](channel-multi-client-user-message-spec.md)、[Approval Lifecycle Spec](approval-lifecycle-spec.md)、[Attachments Support Spec](attachments-support-spec.md)、[Core Runner Turn Flow Spec](core-runner-turn-flow-spec.md)、[Platform Config Restructure Spec](platform-config-restructure-spec.md) 和 [Core Subagent Spec](core-subagent-spec.md)。它们当前为 `Retained Authority`；Slice 6 只做 supersession/link review，不因文件年龄删除。
...
| API-M01 | `RunTurnParams.model/maxTokens`、queue projection：[runtime types](../../src/runtime/types.ts)、[queue types](../../src/runtime/queue-types.ts)；public/runtime Compatibility Candidate | mapping：`model` → Model Reference，`maxTokens` → allowlisted Request Override，queued input 到 start transition 才 resolve。Repository consumers/export surfaces：`runtime/index.ts`、`RuntimeApp.runTurn()`/`runTurnInternal()`、`queue-types.ts`、`prompt-factory.ts`；direct scripts `test-abort-e2e.ts`、`test-abort-live.ts`、`test-runtime-multichannel-integration.ts`、`test-runtime-shutdown-integration.ts`、`test-runtime-wiring-integration.ts`、`test-subagent-e2e.ts`、`test-subagent-live.ts`；`RuntimeApp.test.ts` | Slice 1 Owner；保留为 public Compatibility 到 Slice 2 Review，除非 Slice 1 接受明确 breaking removal；删除需 repository callers 迁移、external consumer notice/decision 和 contract/intake/queue tests |
...
| API-M03 | `AgentRunnerConfig.llmClient`、`RunParams.model/maxTokens/contextWindowTokens`：[Runner types](../../src/core/runner/types.ts)；Active execution Contract / Compatibility Candidate | New Runner Contract 只消费 Resolved Model/core-owned Port。Production consumers/export surfaces：`core/runner/AgentRunner.ts`、`core/runner/index.ts`、`core/subagent/SubagentRunner.ts`、`runtime/bootstrap.ts`、`runtime/RuntimeApp.ts`、`runtime/types.ts`；test helper：`core/runner/test-helpers.ts`；direct integration consumers：`test-agent-runner-compaction-integration.ts`、`test-agent-runner-compaction-reload-integration.ts`、`test-runtime-multichannel-integration.ts`、`test-runtime-queue-integration.ts`、`test-runtime-shutdown-integration.ts`、`test-runtime-steering-integration.ts`；unit consumers：`AgentRunner.test.ts`、`SubagentRunner.test.ts`、`RuntimeApp.intake.test.ts` | Slice 1 Owner（Parent/new Contract），Slice 2 Owner（Child/legacy Contract）；legacy interface 保留为具名 Compatibility 到 Slice 2，且只可映射输入后调用 Resolver；删除需全部 listed callers 迁移、external consumer decision、Runner Contract/build/FT-03/08 |
...
| API-M04 | `LLMClient`、`ChatParams`、`StreamEvent` 和 adapter barrel：[LLM types](../../src/adapters/llm/types.ts)、[barrel](../../src/adapters/llm/index.ts)；Active useful Port 起点但 owner/exports 不符合 Target | Stable Core 拥有 invocation Port/normalized contract；Anthropic Adapter implements。Bounded production caller set：`adapters/llm/AnthropicClient.ts`、`core/media/attachment-pipeline.ts`、`core/runner/AgentRunner.ts`、`core/runner/context/{compaction,context-budget,token-estimation,tool-result-pruning}.ts`、`core/runner/types.ts`、`core/subagent/types.ts`、`runtime/{queue-types,RuntimeApp,subagent-orchestration,summarize-assembled,tool-registry,types}.ts`、`runtime/bootstrap.ts`；script consumers：`test-abort-e2e.ts`、`test-agent-runner-compaction-integration.ts`、`test-agent-runner-compaction-reload-integration.ts`、`test-runtime-attachments-integration.ts`、`test-runtime-queue-integration.ts`、`test-runtime-steering-integration.ts`、`test-runtime-wiring-integration.ts`、`test-subagent-e2e.ts`；unit consumers：`adapters/llm/AnthropicClient.test.ts`、`core/media/attachment-pipeline.test.ts`、`core/runner/AgentRunner.test.ts`、`core/runner/context/{compaction,context-budget,token-estimation,tool-result-pruning}.test.ts`、`runtime/{RuntimeApp,RuntimeApp.intake,summarize-assembled}.test.ts` | Slice 1 Owner；旧 barrel/shape 保留为 Compatibility 到 Slice 2 Review，禁止新增 production caller；删除/收窄需全部 bounded consumers 迁移、external consumer decision、adapter Contract、FT-03/08 和 build |
...
| CODE-M09 | Subagent `llmDefaults`、`inherit`、legacy child params：[subagent orchestration](../../src/runtime/subagent-orchestration.ts)、[SubagentRunner](../../src/core/subagent/SubagentRunner.ts)；Active production path / Compatibility Candidate | Target 是 Parent effective Model Reference + Child independent resolution。Repository callers：`RuntimeApp.create()` shared Runner wiring、public `RuntimeApp.runSubagentTurn()`、`runtime/subagent-orchestration.ts`、Task Tool `core/tools/builtin/task/task-tool.ts`、`test-subagent-e2e.ts`、`test-subagent-live.ts` 及对应 unit tests。Slice 1 adapter 只把 legacy Child inputs 映射给同一 Resolver，不生成 facts/binding | Slice 1 Owner 负责创建、方向约束和初始 Compatibility validation；Slice 1 close 时移交 Slice 2 Owner，后者负责到期和删除。保留到 Slice 2；删除前迁移全部 listed callers，并通过 Subagent model/Usage/Abort/Event/Session tests |
...
| Slice 2 | CODE-M09、API-M03 的 Child Compatibility | Parent/Child 独立 resolution；删除 copied `llmDefaults` 与 legacy Child execution boundary |
