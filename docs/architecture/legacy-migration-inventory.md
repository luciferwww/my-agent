...
|---|---|---|---|
| DOC-A01 | Platform Config Restructure Implementation — `platform-config-restructure-impl.md`（Deleted S6-D4） | verified facts → Current Architecture；durable decisions → ADR；unfinished work 已明确拒绝；其余 narration 经 Review 舍弃 | Capability Inventory；项目所有者 → Slice 6 Owner / Slice 6；config source/tests + links |
| DOC-A02 | Channel Design — `adapters-channel-design.md`（Deleted S6-D4） | verified transport/current facts → Current Channel；target decision 以 Target/Accepted Channel Spec 为准；其余建议已明确拒绝 | WebSocket design、current Runtime/Runner；Slice 6；Channel source/tests + links |
| DOC-A03 | WebSocket Channel Design — `adapters-websocket-channel-design.md`（Deleted S6-D4） | verified protocol/current facts → Current Channel；未接受建议已明确拒绝 | Channel design；Slice 6；WebSocket source/tests + links |
| DOC-A04 | [Subagent Evolution Proposal](core-subagent-evolution-proposal.md)：Reviewed S6-D5，strict Deferred Input | 无 Current/Accepted authority；Project Owner 持有；Foundation freeze 有效；唯一 successor → Architecture Foundation Plan post-Foundation tracker | Target Architecture contextual link；S6-D5 Owner decision + authority/link audit |
| DOC-A05 | [Subagent v2 Spec](core-subagent-v2-spec.md)：Reviewed S6-D5，strict Deferred Input | 文件名不表示 Accepted；Project Owner 持有；Foundation freeze 有效；未来 Accepted Plan/Spec 必须 supersede | Target Architecture contextual link；S6-D5 Owner decision + authority/link audit |
| DOC-A06 | Runner Emit Context Refactor — `core-runner-emit-context-refactor.md`（Deleted S6-D5） | 显式 TurnContext 已进入 Current Runner/source/tests；Owner 确认实现关闭，无剩余批准工作或独有历史价值 | zero inbound；S6-D5 six-question Review + source/test/Git evidence |
| DOC-A07 | Exec Flow Design — `core-tools-builtin-exec-flow-design.md`（Deleted S6-D4） | verified facts → Current Exec section；future work 已明确拒绝；其余 Review | active Tool docs/index；Slice 6；exec/process source/tests + links |
...
|---|---|---|---|
| DOC-A10 | Runtime Design — `runtime-design.md`（Deleted S6-D4） | verified queue/routing/lifecycle facts → Current Runtime；durable target decisions → Accepted Runtime/Composition Specs；其余建议已明确拒绝 | Root README、docs index、v1.0 Runtime/Channel docs；Runtime source、CH-01/02/05–10/12 和 links |
| DOC-A11 | Core Runner Design — `core-runner-design.md`（Deleted S6-D4） | verified execution-loop facts → Current Runner；Tool closure/Compaction decision 分别服从 ADR-001/002；其余建议已明确拒绝 | Root README、docs index、Runtime/Context/Subagent/v1.0 Runner docs；Runner tests、CH-02–04/11/13 和 links |
| DOC-A12 | Core Runner Context Design — `core-runner-context-design.md`（Deleted S6-D4） | verified current budgeting/Compaction facts → Current Runner；target decisions → ADR-002/Accepted Runner Spec；其余建议已明确拒绝 | ADR-002、docs index、Runner/v1.0 docs；Compaction/context tests、CH-04/11 和 links |
| DOC-A13 | Core Runner Hooks Design — `core-runner-hooks-design.md`（Deleted S6-D4） | verified current Hook facts → Current Runner；target Tool/Hook contract → Slice 3 Spec；其余建议已明确拒绝 | Capability Inventory、v1.0 Runner/Channel/Runtime docs；Hook/Runner tests、CH-04/06 和 links |
| DOC-A14 | Platform Config Design — `platform-config-design.md`（Deleted S6-D4） | verified load/source facts → Current Configuration；Model fields ownership → ADR-004/Model Resolution Spec；其余建议已明确拒绝 | Runtime Design、v1.0 Config/Runner docs、Root docs index；loader/defaults/wizard tests 和 links |
| DOC-A15 | Platform Logger Design — `platform-logger-design.md`（Deleted S6-D4） | verified logger facts → Current Observability；未接受 target suggestions 已明确拒绝 | v1.0 Config Wizard/Runtime docs；logger source/tests、FT-01 和 links |
| DOC-A16 | Logging Design — `logging-design.md`（Deleted S6-D4） | verified facts → Current Observability；冲突建议已明确拒绝，不保留双 authority | self-reference and repository link audit；logger source/tests、Owner conflict disposition |
| DOC-A17 | LLM Adapter Design — `adapters-llm-design.md`（Deleted S6-D4） | verified protocol/event facts → Current Provider Adapter；target Port/Facts → Model Resolution Spec/ADR-004；其余建议已明确拒绝 | Root README、docs index、v1.0 Runner；Anthropic adapter tests、FT-03/08 和 links |
| DOC-A18 | Core Prompt Design — `core-prompt-design.md`（Deleted S6-D4） | verified prompt behavior → Current Prompt；未批准的 future Module decision 已明确拒绝 | Root README、docs index、Workspace design；prompt tests、CH-14 和 links |
| DOC-A19 | Core Session Design — `core-session-design.md`（Deleted S6-D4） | verified JSONL/persistence facts → Current Session；未批准的 transaction suggestions 已明确拒绝 | Root README、docs index、Compaction design；Session tests、CH-03/11 和 links |
| DOC-A20 | Core Memory Design — `core-memory-design.md`（Deleted S6-D4） | verified optional/degraded behavior → Current Memory；未批准的 future work 已明确拒绝 | Runtime Design、docs index；Memory tests/resource audit 和 links |
| DOC-A21 | Core Tools Design — `core-tools-design.md`（Deleted S6-D4） | verified Tool contract/execution facts → Current Tools；target Registry/Hook decisions → Slice 3 Spec；其余建议已明确拒绝 | Root README、docs index、Builtin/Subagent v2 docs；Tool tests、CH-03/04/14 和 links |
| DOC-A22 | Builtin Tools Design — `core-tools-builtin-design.md`（Deleted S6-D4） | verified builtin inventory → Current Builtin Tools；未批准的 future Memory Tool work 已明确拒绝 | docs index、Tools/FS docs；production registration、builtin tests 和 links |
| DOC-A23 | Workspace Design — `core-workspace-design.md`（Deleted S6-D4） | verified initialization/loading facts → Current Workspace；未批准的 future decisions 已明确拒绝 | Root README、docs index、Prompt design；Workspace tests 和 links |
| DOC-A24 | Exec Runtime Design — `core-tools-builtin-exec-runtime-design.md`（Deleted S6-D4） | verified platform runtime facts → Current Builtin Tools；未批准的建议已明确拒绝 | docs index、Builtin Tools design；exec/process unit/integration/platform tests 和 links |
| DOC-A25 | Attachments Server Implementation — `attachments-server-implementation.md`（Deleted S6-D4） | durable contract → Attachments Support Spec；verified server facts → Current Media/Channel；过程 narration 可由 Git 重建 | Capability Inventory、Client Implementation；attachment pipeline/Runtime/WebSocket tests 和 links |
| DOC-A26 | Attachments Client Implementation — `attachments-client-implementation.md`（Deleted S6-D4） | durable contract → Attachments Support Spec；verified client facts → Current Media/Channel；过程 narration 可由 Git 重建 | Capability Inventory、Server Implementation；chat client/manual protocol evidence 和 links |
| DOC-A27 | Core Subagent Implementation — `core-subagent-impl.md`（Deleted S6-D4） | durable contract → Core Subagent Spec；verified facts → Current Runtime/Runner；migration work → Slice 2 evidence | Capability Inventory、Subagent Proposal/v2 docs；Subagent unit/e2e、Abort/Usage/Event tests 和 links |
