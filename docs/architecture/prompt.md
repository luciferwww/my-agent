...
`src/core/prompt/` owns deterministic System and User prompt construction and the library-level Context Hook prepender. Runtime supplies the current Context files, visible Tool names, workspace directory, available Subagent summaries, safety settings, and already normalized media blocks.

Prompt does not own configuration precedence, canonical Tool schemas, Channel wire delivery, [media validation and normalization](media.md), [Model Resolution](model-resolution.md), [Provider conversion](providers.md), or [workspace Context-file loading](workspace.md).
...
`runtime/prompt-factory.ts` maps only Runtime-owned inputs into `SystemPromptBuildParams`: prompt mode, effective safety level, Context files, visible Tool names, workspace directory, and available Subagent entries. Even when prompt mode is `none`, Runtime selects the full Context-load mode so its cache remains warm; `minimal` selects minimal Context loading.

Context-file initialization and loading belong to [Workspace](workspace.md). Runtime generation and Subagent lifecycle belong to [Runtime](runtime.md).
