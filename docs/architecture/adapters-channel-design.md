...
> 状态：设计中，待确认
> 范围：Phase 1（CliChannel + WebSocketChannel + TurnInteractionManager）
...
- `TurnInteractionManager`——进程内 Promise bus，统一管理 turn 内阻塞式交互；Phase 1 先以 approval 形态落地
...
| `ChannelInteractionAdapter` | 通用 turn 交互能力（请求、过期、响应） | 多种 interaction adapter 的统一抽象 |
| `ChannelApprovalAdapter` | Phase 1 审批兼容接口 | `ChannelApprovalAdapter` |
| `TurnInteractionManager` | 进程内 Promise bus，连接 hook 与 channel | `ExecApprovalManager` |
...
AgentRunner 调用 tool 前
   ...
   RuntimeApp 注册的 hook handler:
        ...
        └─ turnInteractionManager.request({ toolName, input, sessionKey, originClientId })
                │ 阻塞等待 Promise
                │
 ...
 * 通用 turn 交互适配器。
 *
 * `approval` 可以视为它的一个特例：Phase 1 沿用老接口，后续新交互优先走这里。
 */
...
## 4. TurnInteractionManager（`src/adapters/channel/TurnInteractionManager.ts`）

进程内 Promise bus，统一管理 turn 内阻塞式交互。当前实现仍只落地 approval，但 RuntimeApp 和 channel 层的路由边界已经提升到更一般的 interaction 语义。

> 兼容说明：`src/adapters/channel/index.ts` 目前仍导出 `TurnInteractionManager as ApprovalManager`，仅用于兼容旧调用点；本文以下统一使用 `TurnInteractionManager` 作为正式名称。
...
**关键设计：interaction 按 turn 起源路由（不广播）**
...
export interface WebSocketChannelConfig {
  ...
  /** 启用 Phase 1 approval 交互；内部同时打开 interaction/approval 双轨兼容 */
  approval?: boolean;   // 默认 false
}
...
客户端完成 `hello(clientId)` 后，该逻辑客户端与当前 websocket 连接绑定。之后客户端发送 `run_turn` 时，channel 将该 `clientId` 写入 `ChannelRunRequest`，同时把该 clientId 注册进 sessionKey 对应的集合。若同一 `clientId` 后续重新连接，服务端采用“后连覆盖前连”策略，将逻辑客户端重新绑定到新连接，并立即主动关闭旧连接。连接断开时，`WebSocketChannel` 会立即清理自身维护的 transport 路由状态。若未来上层补上按 `clientId` 管理的 pending interactions 重投递钩子，可在新的 `hello(clientId)` 成功后再次下发交互；这属于上层交互管理，不属于 websocket transport 恢复。

关键时序可参考 [adapters-websocket-channel-design.md](./adapters-websocket-channel-design.md) 中的 `hello(clientId)` 接管图、`run_turn` 广播路径图、断线清理图，以及标注为 future 的 pending interactions 重投递图。
...
class RuntimeApp {
  ...
  /**
   * 依次调用所有已注册 channel 的 stop()；不在此处关闭 TurnInteractionManager。
   * close() 内部会先 stopChannels()，再关闭上层交互 manager。
   */
...
src/adapters/channel/
  types.ts              # Channel / ChannelInteractionAdapter /
                        # ChannelApprovalAdapter / TurnInteraction* / Approval*
  TurnInteractionManager.ts  # Promise bus（index.ts 仍兼容导出 ApprovalManager 别名）
...
新增：
- 内部字段：`turnInteractionManager: TurnInteractionManager`、`channels: Channel[]`、`originChannelByTurn: Map<string, Channel>`、`originClientByTurn: Map<string, string>`、`approvalRoutingWired: boolean`、`inFlightSessions: Set<string>`
