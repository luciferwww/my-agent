...
> 创建日期：2026-04-02  
> 参考：相关 Session 管理系统分析（详见 [openclaw-session-analysis.md](../analysis/openclaw/openclaw-session-analysis.md)）
...
> 这些能力保持分层：Session 负责存储和读取，会话裁剪、压缩和上下文窗口控制仍属于更上层的执行逻辑。
...
- 使用 `sessions.json` + `{sessionId}.jsonl` 两层结构；
- transcript 使用树形 `parentId`，支持分支；
- 使用 per-file Promise 队列处理并发写入；
- 暂不做 store 缓存、归档和维护清理。
...
> - `'toolResult'`：工具执行结果
>
> `toolResult` 使用独立 role 存储，目的是把工具结果和普通用户消息明确区分；发送给 Anthropic API 时再由 agent-runner 转换为 `role: 'user'` + `{ type: 'tool_result', ... }` content block。
>
...
> **为什么不存 system prompt**：system prompt 由 prompt-builder 每次实时生成，因为时间、工具、contextFiles 这些输入都可能变化。Session 只保存消息历史，不保存每次调用时生成的 system prompt 文本。
...
说明：
- 当前不对 Session Key 施加严格命名规则；
- 调用方只要保证同一语义下 key 稳定即可。
