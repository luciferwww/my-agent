...
Accepted。项目所有者于 2026-09-01 确认 CH-04 的 Characterization 与 migration disposition：当前 after Tool 与 compaction observer Hooks detached、不阻塞 Turn settlement 的行为作为迁移前事实保留；目标 `after_tool_call` 为 observer-only，不得改变 canonical Tool Result，handlers 并行执行且逐个失败隔离，但 Runner 必须在下一次 Model invocation 或 Turn completion 前等待整组 `allSettled`，不得越过 Turn completion 或 Snapshot pin。`before_tool_call` 继续按确定顺序 awaited，使后续 Hook 观察前序 effective input，并在拒绝或失败时终止执行。Compaction observer 的目标 settlement 由 [ADR-002](../architecture/adr-002-context-budgeting-and-compaction-recovery.md) 约束，不与 Tool Hook Contract 合并。

项目所有者于 2026-09-01 确认 CH-05 的 Characterization 与 migration disposition：当前 `onAgentEvent` callback 抛错会向上传播并阻止 Runner 启动的行为作为迁移前事实保留；目标 Agent event fanout 按 target 隔离，单个 observer 失败只产生可关联诊断，不影响其他 target、不阻止 Runner，也不改变 Turn result。需要控制或拒绝 Turn 的逻辑必须使用明确的 Policy 或 modifying Hook Contract，不得借 observer failure 隐式取得控制权。

项目所有者于 2026-09-01 确认 CH-06 的 Characterization 与 migration disposition：现有 deny/allow/prompt matrix、无 capability fail-closed、`startChannels()` startup-history dependency 和 capable origin 的隐藏固定 120 秒 timeout-deny 均作为迁移前事实保留；目标移除默认 120 秒 timeout-deny，人工 approval 采用 response-or-abort，并观察 Turn Abort、Shutdown 和当前调用 capability 可用性。未来 `Allow all`、`Always allow` 或其他持久/会话级授权机制暂不讨论，不属于本 disposition。

Batch 2（CH-04、CH-05、CH-06）的 Characterization 与 migration disposition 已接受。本节不授权 Phase 1 Batch 3 或 CH-04/CH-05 生产代码变更。CH-04 生产迁移必须先有 Accepted Tool Hook Module Spec，并作为独立 Architecture Slice 获得 Delivery 批准；CH-05 生产迁移同样需要独立的 Accepted Contract/Spec 与 Delivery 批准。

项目所有者于 2026-09-01 接受 [Approval Lifecycle Module Spec](../architecture/approval-lifecycle-spec.md) v0.1，并单独批准 CH-06 Approval Lifecycle Architecture Slice 进入 Delivery。该次独立授权只覆盖移除默认 120 秒 timeout-deny、接入 response-or-abort 生命周期及所需 Channel/client/test 迁移；CH-04/CH-05 disposition 随后另行接受，但仍未获得生产迁移授权。该授权不包含 Phase 1 Batch 3、`Allow all`、`Always allow` 或其他持久/会话级授权机制。

### Phase 1 Batch 3 evidence

| Scope | Command | Result |
|---|---|---|
| CH-07 | VS Code Test Runner：`CH-07` | Pass；1 test；injected start rejection + start/stop counters |
| CH-08 | VS Code Test Runner：`CH-08` | Pass；2 tests；deferred nonresponsive Runner、public queued ingress 与 rejecting Channel stop |
| CH-09 | VS Code Test Runner：`CH-09` | Pass；2 tests；public Channel ingress、event recorder 与 unread steering cleanup |
| Related regression | VS Code Test Runner：`RuntimeApp.test.ts` | Pass；23 tests |
| TypeScript baseline | `npx tsc --noEmit` | Pass；exit 0 |
| Patch hygiene | `git diff --check` | Pass；exit 0 |

Batch 3 只修改 Characterization Tests，没有修改 production code、公共 Contract 或 Accepted Target Architecture，也没有新增 dependency 或 production test seam。为避免无意义重复，CH-08 nonresponsive/queued 与 CH-09 active+queued 场景通过强化既有测试完成；仅对 CH-07 partial start、CH-08 Channel stop failure 和 CH-09 unread steering cleanup 新增独立测试。所有新增观察均使用 public Channel ingress、Runner signal/call count、Runtime/Agent events 和 Shutdown Report，不读取 RuntimeApp 私有状态。

### Phase 1 Batch 3 CH-07 Owner disposition

Accepted。项目所有者于 2026-09-02 接受修订后的 CH-07 migration disposition：不存在 required Channel，也不存在 required Extension；独立 Channel 跨实例启动允许部分成功，零 Channel 启动成功也不阻止核心 Agent Runtime 启动。单个 Channel 启动失败只产生可定位 warning 与 degraded diagnostics，不阻止其他已成功 Channel 或核心 Agent 继续运行。失败 Channel 必须清理本次启动创建的 partial resources，单个 Channel 内保持 atomic startup；Shutdown 只停止实际启动的 Channel，并保持 close-once。核心 Runtime composition、内部一致 Snapshot 或其他非 Channel 的显式必需依赖失败仍 fail-fast。External Extension 加载/校验失败继续按 §6 整组隔离而不使 Agent 启动失败；其他 Contribution 的运行期启动语义及 per-Channel restart/stop/start 控制留待后续独立设计。
...
Accepted。项目所有者于 2026-09-02 接受修订后的 CH-08 two-stage bounded Shutdown migration disposition：Stage 1 立即停止接收新 Root/Child request，经 public completion gate 终结 queued/unstarted request 和 approval/interaction/capability wait，同时保持 Channel/Fanout 可用，并给其余已运行 Turn tree 一个独立的 bounded graceful-drain 窗口；Stage 2 在 drain deadline 到达后向剩余 Turn tree 传播 Abort，并在独立的 bounded Abort-convergence deadline 内等待 Hook、Tool、Provider、Child Turn 和 Snapshot pin 收敛。Abort-convergence 到期后，对外 request 必须恰好一次结算为 classified `shutdown/non-converged`，并报告 residual worker/pin；停止等待不授权 RuntimeApp 或 Builder 强制关闭仍受 pin/使用者保护的资源。单个组件的 `stop()` 失败不得阻止其他 eligible 组件清理；重复 `close()` 必须共享同一 Shutdown 过程，owned resources 保持 close-once；最终结果记录 graceful、aborted、non-converged、failed、timed-out 与 protected residual diagnostics。`RuntimeApp` 不直接调用 `process.exit()`；overall deadline、第二次 forceable signal 和最终进程终止只由 Runtime host 决定。
...
Accepted。项目所有者于 2026-09-02 接受修订后的 CH-09 Abort 与 late-steering migration disposition：保留 active Root Turn Abort、同 Session queued/followup messages 清空、cross-session isolation、Child Turn signal cascade 与 `abortTurn()` never-throw 语义；每个已启动 Turn 必须恰好产生一次 completion，重复 Abort 不得产生第二次 completion。

现在不引入逐条 steering terminal settlement。Steering admission 只表示输入进入入站时 active Turn 的 steering inbox，不承诺 Runner 或模型最终消费；现有 `user_message` 广播及其 `deliveryMode: 'steering'` 同样不能作为消费证明。因 Turn 正常结束、Abort、失败或 Shutdown 而未读取的 steering 输入必须清理，不得自动转成下一条 queued Root request；聚合数量和分类原因只需进入 diagnostics。该目标不要求逐条 `consumed`/`dropped` 公共事件、settlement ledger、持久化或 completion gate；Turn 级 exactly-once terminal outcome 保持不变。未来如远程 Channel 需要可靠 receipt，必须由独立 Accepted Contract 将 typed `accepted`/`rejected` 定义为 admission result，不得把它表述为模型 consumption。
