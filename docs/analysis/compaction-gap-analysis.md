...
> 初版日期：2026-04-16  
> 最后更新：2026-04-16（Gap 1 已设计完成，升入实施阶段）  
...
| prompt 独立传入 | messages + prompt 分开，delay-append | 同（current message 在预算检查通过后才 append） |
...
**路由阈值**（与 OpenClaw 完全对齐）：
```typescript
// context-budget.ts（内部常量）
const CHARS_PER_TOKEN = 4;               // 参考 OpenClaw ESTIMATED_CHARS_PER_TOKEN
const TRUNCATION_BUFFER_TOKENS = 512;    // 参考 OpenClaw TRUNCATION_ROUTE_BUFFER_TOKENS
...
**实施状态**：设计文档（`compaction-design.md`）已更新，源码实施中（Phase 1）。

---

## 待解决差异

### Gap 2：tool result 裁剪范围（内存 vs 持久化）

| | OpenClaw | my-agent |
|--|--|--|
| 裁剪作用范围 | `truncateOversizedToolResultsInSession` — **修改 session 文件** | `pruneToolResults` / `pruneToolResultsAggregate` — **仅修改内存中的 messages** |
| 下次 turn 是否生效 | ✅ 一次裁剪，永久生效 | ❌ 每次 turn 重新加载原始数据，重复裁剪 |

**影响**：my-agent 中，大 tool result 每轮都要重新裁剪，有重复计算开销；且 token 估算基于原始未裁剪数据，可能偏高。

**建议**：初期可接受，后续可作为优化项：Layer 1/1.5 命中时同步写回 session 文件。
...
| Gap | 严重性 | 状态 | 建议优先级 |
|-----|-------|------|----------|
| ~~Gap 1：缺少 truncate_tool_results_only 路由~~ | 中 | **实施中（Phase 1）** | — |
| Gap 2：tool result 裁剪只在内存 | 低（重复开销，初期可接受） | 待处理 | 优化项 |
