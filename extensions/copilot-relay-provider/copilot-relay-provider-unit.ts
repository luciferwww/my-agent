...
async function discoverModels(
  ...
  try {
    ...
    const discoveryURL = `${baseURL}/v1/models`;
    let response: Response;
    try {
      response = await (options.fetch ?? globalThis.fetch)(discoveryURL, {
        method: 'GET',
        headers: {
      ...
      throw new Error(
        `Copilot Relay model discovery request to "${discoveryURL}" failed: ${detail}`,
        { cause: error },
      );
    }
    if (!response.ok) {
